...
async function run() {
  const result = await flexprice.rbac.listRoles();
...
import { rbacListRoles } from "@flexprice/sdk/funcs/rbacListRoles.js";
...
async function run() {
  const res = await rbacListRoles(flexprice);
  if (res.ok) {
...
async function run() {
  const result = await flexprice.rbac.getRole("<id>");
...
```typescript
import { FlexpriceCore } from "@flexprice/sdk/core.js";
import { rbacGetRole } from "@flexprice/sdk/funcs/rbacGetRole.js";
...
async function run() {
  const res = await rbacGetRole(flexprice, "<id>");
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
