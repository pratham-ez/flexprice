 ...
 * @generated-id: 499a1e55da7e
 */

import { FlexpriceDefaultError } from "../models/errors/flexpricedefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | FlexpriceDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | FlexpriceDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new FlexpriceDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new FlexpriceDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
